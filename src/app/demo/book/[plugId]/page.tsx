// src/app/demo/book/[plugId]/page.tsx
// Screen 2 — Book a job. Pick amount + describe the work, then create a hack_jobs row
// and move to the escrow payment screen.
...
import { Card, Label, TextInput, TextArea, PrimaryButton, Money } from '../../_components/ui';
import { jsonFetch, getDemoIdentity } from '../../_lib/demo';
...
export default function BookPage() {
  ...
  async function book() {
    ...
    try {
      ...
      const { job } = await jsonFetch('/api/jobs', {
        ...
        body: JSON.stringify({
          plugId,
          clientName: name || 'Demo Client',
          clientPhone: phone,
          jobDescription: description.trim(),
          amount: amt,
        }),
      });
      router.push(`/demo/pay/${job.id}`);
    } catch (e: any) {
      setError(e.message);
      setSubmitting(false);
    }
  ...
  return (
    <Shell
      eyebrow="Step 2 · Book"
      title="Book this Plug"
      back="/demo/browse"
      footer={
        <PrimaryButton onClick={book} loading={submitting}>
          {submitting ? 'Creating job…' : 'Continue to payment'}
    ...
    >
      {loadError && (
        <Card className="p-4 mb-6">
          <p className="text-sm text-slate">Couldn’t load this Plug: {loadError}</p>
        </Card>
      )}
      ...
      {plug && (
        <Card className="p-4 mb-6 flex items-center gap-4">
          <div className="relative shrink-0">
            <div className="grid place-items-center h-12 w-12 rounded-2xl bg-midnight text-white font-display text-lg">
              {plug.name?.[0]}
            </div>
            {plug.verified && (
              <span className="absolute -bottom-1 -right-1 grid place-items-center h-5 w-5 rounded-full bg-white">
      ...
      <div className="space-y-5">
        <div>
          ...
          <TextArea
            ...
            rows={3}
            placeholder="e.g. Faulty DB board diagnosis and repair"
          />
        </div>
