    ...
    <Shell eyebrow="Plug · Withdraw" title="Withdraw to bank" back={`/app/wallet/${plugId}`}>
      {done ? (
        <Card className="p-7 text-center">
          ...
          <p className="mt-2 text-sm text-slate leading-relaxed"><span className="font-bold text-pitch-black">{naira(amount)}</span> is queued for payout. Payouts are bank-initiated, so this stays <span className="font-bold text-pitch-black">pending</span> — we don’t fake a completed transfer.</p>
