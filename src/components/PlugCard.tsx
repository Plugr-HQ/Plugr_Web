  ...
  return (
    <div className={cn("w-full bg-white rounded-3xl shadow-sm p-5 border border-gray-100 max-w-[420px]", className)}>
      ...
      <div className="flex items-center justify-between gap-4 pt-1">
        <Link
          href={`/p/${plug.id}`}
          className="text-base text-[12px] font-bold text-[#DBA134] hover:text-[#DBA134]/90 transition-colors"
        >
          View Profile
        </Link>
        ...
        <button
          ...
          className="bg-[#DBA134] hover:bg-[#DBA134]/90 text-[#0A1529] font-bold px-3 py-2 rounded-full flex items-center gap-1 transition-colors text-sm shadow-sm"
        >
          {/* Custom WhatsApp Icon Path */}
          <FaWhatsapp className='w-5 md:w-7 h-5 md:w-5' />
          <span className='text-[12px]'>Request this Plug</span>
        </button>
      </div>
    </div>
  )
